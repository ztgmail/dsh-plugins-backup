...
export function validateManifest(m) {
    ...
    if (!exporter || typeof exporter !== 'object') {
        ...
        if (typeof exporter['name'] !== 'string')
            issues.push({ path: 'exporter.name', message: 'exporter.name 必须是字符串', severity: 'error' });
        if (typeof exporter['version'] !== 'string')
            issues.push({ path: 'exporter.version', message: 'exporter.version 必须是字符串', severity: 'error' });
    }
    ...
    if (!source || typeof source !== 'object') {
        ...
        if (typeof source['dshVersion'] !== 'string')
            issues.push({ path: 'source.dshVersion', message: 'source.dshVersion 必须是字符串', severity: 'error' });
        if (typeof source['platform'] !== 'string')
            issues.push({ path: 'source.platform', message: 'source.platform 必须是字符串', severity: 'error' });
        if (typeof source['arch'] !== 'string')
            issues.push({ path: 'source.arch', message: 'source.arch 必须是字符串', severity: 'error' });
    }
    ...
    if (!security || typeof security !== 'object') {
        ...
        if (typeof security['containsSecrets'] !== 'boolean')
            issues.push({ path: 'security.containsSecrets', message: 'containsSecrets 必须是布尔', severity: 'error' });
        if (typeof security['encrypted'] !== 'boolean')
            issues.push({ path: 'security.encrypted', message: 'encrypted 必须是布尔', severity: 'error' });
