...
export declare const TOMBSTONES_FILE = "tombstones.json";
...
export declare function addTombstone(list: Tombstone[], entry: Tombstone): Tombstone[];
/** 某条目（kind+id）是否已被墓碑覆盖 */
export declare function isTombstoned(kind: TombstoneKind, id: string, tombstones: readonly Tombstone[]): boolean;
/** 过滤 {kind,id} 形态数组：剔除墓碑命中的条目，保留其余（返回新数组，不改原数组）。
 *  非 TombstoneKind 的 kind（如导入计划项的 Install/Update 等）永远不会误命中。 */
export declare function filterTombstoned<T extends {
    kind: string;
    id: string;
}>(items: readonly T[], tombstones: readonly Tombstone[]): T[];
/** 读取 <dataDir>/tombstones.json；文件不存在 / 解析失败 / 结构非法 → []（安全降级，不抛错） */
export declare function loadTombstones(fs: TombstoneFs, dataDir: string): Promise<Tombstone[]>;
/** 写入 <dataDir>/tombstones.json（覆盖写；dataDir 目录由调用方保证可写） */
export declare function saveTombstones(fs: TombstoneFs, dataDir: string, list: Tombstone[]): Promise<void>;
