...
import { withMutationLock } from "../utils/env-lock.js";
import { readAutosyncConfig, writeAutosyncConfig } from "./autosync-config.js";
import { readSyncConfigFor, isGitConfig, isWebDavConfig } from "./sync-config.js";
...
export class AutoSyncScheduler {
    ...
    async appendHistory(channel, entry) {
    ...
    async runOnce(channel, opts = {}) {
        ...
        try {
            ...
            if (remoteNew) {
                ...
                if (apply.autoApply.length === 0) {
                    ...
                    const applyReport = (this.phase3Recovery !== undefined && this.lockCtxForJournal !== null)
                        ...
                        : await rawApply();
                    appliedSections = applyReport.applied;
                    if (!applyReport.ok) {
            ...
            const result = {
                ...
                ...(appliedSections.length === 0 ? { skipReason: 'unchanged' } : {}),
    ...
    /** 收尾：写该通道 autosync-config（lastRunAt, lastRunStatus, consecutiveFailures, lastRunHistoryId）。 */
    async writeFinalConfig(base, result, nowIso, historyId, channel) {
    ...
    maybeNotify(failures, historyId, nowIso, channel) {
        if (failures >= 3) {
 ...
 */
export function syncIsConfigured(cfg) {
 ...
 */
async function defaultDetectRemoteNew(engine) {
    const fn = engine.hasNewRemoteSnapshot;
 ...
 */
async function defaultDetectLocalChange(engine) {
    const fn = engine.hasLocalChanges;
    if (typeof fn === 'function') {
        try {
            return await fn.call(engine);
        }
