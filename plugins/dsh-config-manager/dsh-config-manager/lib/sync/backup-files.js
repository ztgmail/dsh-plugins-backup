...
export function backupFileSource(name) {
...
export function isValidExportFileName(name) {
 ...
 * - 集合不含该名 → 原样返回（零额外开销）。
 */
export function resolveNonCollidingExportName(desiredName, existingNames) {
...
export async function readBackupNotes(exportsDir) {
    try {
...
export async function writeBackupNote(exportsDir, name, note) {
 ...
 * 目录缺失/不可读 → 返回空数组（不抛错）。
 */
export async function listBackupFiles(exportsDir) {
    let entries;
    try {
 ...
 */
export async function deleteBackupFile(exportsDir, name) {
 ...
 */
export async function pruneAutoBackups(exportsDir, keep) {
