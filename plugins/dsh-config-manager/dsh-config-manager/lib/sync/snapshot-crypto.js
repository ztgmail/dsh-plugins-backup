...
import { sectionsFromJsonSafe, sectionsToJsonSafe } from "./snapshot-json.js";
/**
 ...
 */
export async function encryptSectionsPayload(sections, password) {
    if (password === '')
...
export async function decryptSectionsPayload(payload, password) {
    if (password === '')
    ...
    return sectionsFromJsonSafe(parsed);
}
/** 从加密载荷提取非秘密的加密参数（展示/诊断用；不含密码）。 */
export function encryptionInfoOf(payload) {
    return payload.info;
}
//# sourceMappingURL=snapshot-crypto.js.map