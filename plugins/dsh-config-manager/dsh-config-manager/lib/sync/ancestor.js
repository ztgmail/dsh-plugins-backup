...
import { readSnapshotFromDir, writeSnapshotToDir } from "./layout.js";
import { SNAPSHOT_MANIFEST_FILE } from "./layout.js";
/** 默认保留的祖先快照数量（最旧超出则被裁剪）。 */
export const DEFAULT_ANCESTOR_KEEP = 10;
/** 按 snapshotId 读取祖先快照目录；不存在抛错，不静默降级。 */
export async function loadAncestor(localSnapshotsDir, snapshotId, fsx = createSnapshotFs()) {
...
export async function writeAncestor(localSnapshotsDir, snapshot, fsx = createSnapshotFs()) {
...
export async function listAncestors(localSnapshotsDir, fsx = createSnapshotFs()) {
    if (!(await fsx.exists(localSnapshotsDir)))
        return [];
...
export async function pruneAncestors(localSnapshotsDir, keep = DEFAULT_ANCESTOR_KEEP, fsx = createSnapshotFs()) {
    const all = await listAncestors(localSnapshotsDir, fsx);
    if (keep <= 0 || all.length <= keep)
        return [];
