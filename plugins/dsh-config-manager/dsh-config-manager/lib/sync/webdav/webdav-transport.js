...
import { computeSnapshotMeta, sectionsEqual } from "../transport.js";
...
const defaultRequest = async (method, url, options = {}) => {
...
export class WebDavTransport {
    ...
    constructor(options) {
        ...
        if (options.request !== undefined)
            this.o.request = options.request;
        if (options.timeoutMs !== undefined)
            this.o.timeoutMs = options.timeoutMs;
        if (options.msg !== undefined)
            this.o.msg = options.msg;
    }
    ...
    parseIndex(raw, url, pwd) {
        let parsed;
        try {
            parsed = parseJsonSafe(raw);
        }
        catch (err) {
            throw new WebDavTransportError(this.o.msg('sync.webdav.indexInvalid', { url, err: this.mask(String(err?.message ?? ''), pwd) }));
        }
        if (!Array.isArray(parsed)) {
            throw new WebDavTransportError(this.o.msg('sync.webdav.indexInvalid', { url, err: 'not an array' }));
        }
        const valid = (m) => typeof m === 'object' && m !== null
            && typeof m.id === 'string'
            && typeof m.createdAt === 'string'
            && typeof m.manifest === 'object' && m.manifest !== null
            && typeof m.sections === 'object' && m.sections !== null;
    ...
    parseSnapshot(raw, id) {
        try {
            return deserializeSnapshot(raw);
        }
        catch (err) {
            throw new WebDavTransportError(this.o.msg('sync.webdav.snapshotInvalid', { id, err: this.mask(String(err?.message ?? ''), '') }));
        }
    }
    /** 发送请求：注入 Basic 认证头 + 调用 request；网络错误/超时归一 */
    async send(method, url, pwd, opts = {}) {
        ...
        try {
            ...
            if (this.isTimeout(err)) {
                throw new WebDavTransportError(this.o.msg('sync.webdav.timeout', { method, url, timeout: String(this.o.timeoutMs) }));
            }
            if (err instanceof Error && err.name === 'RedirectError') {
                // 默认 request 的重定向跳数超限（如 302 循环）→ 归一为清晰消息，避免暴露内部跳转详情
                throw new WebDavTransportError(this.o.msg('sync.webdav.tooManyRedirects', { method, url, n: String(MAX_REDIRECTS) }));
            }
            const rawMsg = err instanceof Error
                ? (err.cause ? `${err.message} (${err.cause.message || err.cause})` : err.message)
                : String(err);
            throw new WebDavTransportError(this.o.msg('sync.webdav.requestError', { method, url, err: this.mask(rawMsg, pwd) }));
        }
    }
    /** 构造 HTTP 非 2xx 失败消息：附上脱敏后的响应体片段（截断） */
    async failText(method, url, res, pwd) {
        let body = '';
        try {
...
//# sourceMappingURL=webdav-transport.js.map