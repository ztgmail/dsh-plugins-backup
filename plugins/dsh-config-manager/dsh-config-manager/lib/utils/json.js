...
export function measureDepth(value) {
    let max = 0;
    // 栈元素：[value, depth]
    const stack = [[value, 1]];
...
export function parseJsonSafe(text, opts = {}) {
    ...
    try {
        ...
        if (err instanceof RangeError)
            throw new JsonDepthError(`JSON 解析失败（可能是嵌套过深）: ${String(err.message)}`);
        throw err;
    }
...
export function stringifyJsonSafe(value, opts = {}) {
    const maxDepth = opts.maxDepth ?? DEFAULT_MAX_JSON_DEPTH;
...
/** 深度保护深拷贝（迭代式，安全） */
export function deepClone(value, maxDepth = DEFAULT_MAX_JSON_DEPTH) {
    const depth = measureDepth(value);
    if (depth > maxDepth)
