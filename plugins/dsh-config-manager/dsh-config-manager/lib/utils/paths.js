...
export function isSameOrChild(p, parent) {
 ...
 */
export function normalizePathCollapsed(p) {
    const norm = normalizePath(p);
    if (norm === '')
        return '';
    if (norm === '.')
...
export function isReservedInternalRel(relPath) {
 ...
 */
export function applyPrefixMappings(value, mappings) {
 ...
 */
export function collectAbsolutePaths(value, prefix = '') {
    const hits = [];
    const visit = (v, p) => {
        if (typeof v === 'string') {
            if (isAbsolutePath(v))
                hits.push({ value: v, path: p });
            return;
        }
