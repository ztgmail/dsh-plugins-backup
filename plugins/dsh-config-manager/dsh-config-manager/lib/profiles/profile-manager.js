...
import { ImportNotConfirmedError } from "../core/types.js";
import { isFileSection } from "../schema/config.js";
import { stringifyJsonSafe, parseJsonSafe } from "../utils/json.js";
...
export function decodeSections(encoded) {
    const sections = new Map();
    for (const [sectionId, data] of Object.entries(encoded)) {
...
export class ProfileManager {
    ...
    async list() {
        let entries = [];
        try {
            entries = await fs.readdir(this.profilesDir, { withFileTypes: true }).then((es) => es.filter((e) => e.isDirectory()).map((e) => e.name).sort());
        }
        catch {
            return []; // 目录不存在视为空
        }
    ...
    async meta(name) {
        ...
        const sections = Object.keys(stored.sections);
        return {
            name: stored.name,
    ...
    async readStored(name) {
        if (!isValidProfileName(name))
            throw new Error(`非法 Profile 名: "${name}"`);
        let raw;
        try {
    ...
    async saveCurrent(name, opts = {}) {
        ...
        for (const adapter of this.adapters) {
            ...
            try {
                ...
                if (!isEmpty)
                    sections.set(adapter.id, section.data);
            }
            catch (err) {
    ...
    async duplicate(name, newName) {
        if (!isValidProfileName(newName))
            throw new Error(`非法新 Profile 名: "${newName}"`);
        const stored = await this.readStored(name);
        const now = new Date().toISOString();
        const copy = {
            ...
            try {
                ...
                for (const issue of v.issues) {
                    if (issue.severity === 'error')
                        issues.push(`${adapter.id}: ${issue.message}`);
                }
                if (!v.valid)
                    continue;
    ...
    async exportProfile(name, outPath) {
        const stored = await this.readStored(name);
    ...
    async importProfile(filePath, opts = {}) {
        let raw;
        try {
        ...
        if (await this.exists(name))
            throw new Error(`Profile "${name}" 已存在，请先删除或指定 asName`);
        const now = new Date().toISOString();
        const stored = {
            name,
            version: 1,
...
function applyResolution(item, strategy, resolutions) {
...
function buildProfileManifest(ctx) {
    return {
        schemaVersion: 1,
