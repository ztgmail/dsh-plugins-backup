...
export function computeItemBadge(detail, t) {
 ...
 */
export function marketItemWarnings(manifest, url, downloadedAt, t) {
    const warnings = [];
    warnings.push(t('market.supplyUnofficial'));
    if (url !== '')
        warnings.push(t('market.supplySource', { url }));
    if (downloadedAt !== '')
        warnings.push(t('market.supplyDownloadedAt', { time: downloadedAt }));
    if (manifest?.author)
        warnings.push(t('market.supplyAuthor', { author: manifest.author }));
    if (manifest?.provenance?.source)
        warnings.push(t('market.supplyProvenanceSource', { source: manifest.provenance.source }));
    if (manifest?.provenance?.note)
        warnings.push(t('market.supplyProvenanceNote', { note: manifest.provenance.note }));
    return warnings;
}
/**
 * 是否需要人工决策（供应链警示模型层恒 true —— 不让默认信任来源，须逐项确认）。
 */
export function needsReview(_detail) {
    return true;
}
/** 从 MarketIndexItem 投影浏览列表项（cacheState 由调用方按缓存状态计算；repo 透出供来源徽章） */
export function toMarketListItem(item, cacheState) {
    return {
        id: item.id,
        name: item.name,
