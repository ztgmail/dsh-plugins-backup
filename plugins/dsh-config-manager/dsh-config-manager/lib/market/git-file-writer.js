...
import { validateRepoUrl } from "../sync/sync-config.js";
import { atomicWriteFile } from "../utils/atomic-write.js";
const execFileAsync = promisify(execFile);
const DEFAULT_TIMEOUT_MS = 60_000;
const DEFAULT_AUTHOR = { name: 'DSH Config Manager', email: 'publish@dsh.local' };
...
function makeMask(repoUrl, token) {
    return (text) => {
...
function validateAndNormalizeRepoUrl(repoUrl) {
    if (typeof repoUrl !== 'string' || repoUrl.trim() === '') {
        throw new GitFileWriterError('仓库地址不能为空');
    }
    const err = validateRepoUrl(repoUrl);
    if (err !== null)
        throw new GitFileWriterError(`仓库地址非法: ${err}`);
...
export class GitFileWriterClient {
    credentials;
    gitBin;
    timeoutMs;
    exec;
     ...
     */
    async buildCredentialArgs(token) {
    ...
    async ensureRepo(repoUrl, workDir, token, mask) {
    ...
    async ensureUpstream(workDir, upstreamUrl, mask) {
        const token = await this.credentials.getToken();
    ...
    async detectDefaultBase(workDir, token, mask) {
...
export function createGitFileWriter(options) {
    return new GitFileWriterClient(options);
}
//# sourceMappingURL=git-file-writer.js.map