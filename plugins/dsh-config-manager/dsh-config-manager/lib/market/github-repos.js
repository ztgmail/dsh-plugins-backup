...
function classifyErrorCode(status, rateLimited) {
    if (status === 401)
        return 'unauthorized';
    if (status === 403)
        return rateLimited ? 'rate_limited' : 'forbidden';
    if (status === 404)
        return 'not_found';
    if (status === 409)
        return 'conflict';
...
export class GitHubAuthRest {
    ...
    async createPublicRepo(name, description) {
        const body = { name, private: false, auto_init: true };
    ...
    async readFile(owner, repo, path, ref) {
        ...
        const response = await this.request(`/repos/${seg(owner)}/${seg(repo)}/contents/${segPath(path)}${query}`, { method: 'GET' });
        if (response.status === 404)
            return null;
        if (!response.ok)
            throw await this.apiError(response);
     ...
     */
    async getRepoStarsPublic(owner, repo) {
    ...
    async openPullRequest(params) {
        ...
        const body = { title: params.title, head: params.head, base: params.base };
    ...
    async listOpenPullRequests(owner, repo, head) {
     ...
     */
    async findOwnFork(owner, repo, login) {
    ...
    async tryReadOwnFork(response, owner, repo) {
        const data = await safeParseJson(response);
        const obj = asRecord(data);
        const isFork = getBool(obj, 'fork');
        const parentName = parentFullNameOf(obj);
        if (isFork !== true || parentName !== `${owner}/${repo}`) {
            throw new GitHubApiError('你的同名仓库已存在但不是本市场的 fork，无法自动收录，请改名或删除后重试', 'fork_name_conflict');
        }
        const fullName = getStr(obj, 'full_name');
        return {
            fullName,
    ...
    async waitForkReady(login, repo, owner) {
    ...
    async tryReadForkInfo(response, owner, repo) {
        try {
            const data = await safeParseJson(response);
            const obj = asRecord(data);
            ...
            const fullName = getStr(obj, 'full_name');
            const parentName = parentFullNameOf(obj);
            if (login === undefined || getBool(obj, 'fork') !== true || parentName !== `${owner}/${repo}`)
                return null;
            return {
                fullName,
                htmlUrl: getStr(obj, 'html_url'),
    ...
    async request(path, init = {}) {
        ...
        try {
            ...
            throw new GitHubApiError(`GitHub API 请求失败：${redact(err instanceof Error ? err.message : String(err))}`, 'network_error');
        }
        return response;
    }
    /**
