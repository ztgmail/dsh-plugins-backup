...
export async function readMarketConfig(dir) {
    ...
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed))
        return emptyMarketConfig();
    const obj = parsed;
    if (obj['schemaVersion'] !== MARKET_CONFIG_SCHEMA_VERSION)
        return emptyMarketConfig();
    ...
    for (const m of markets) {
        if (m === null || typeof m !== 'object' || Array.isArray(m))
            continue;
        const mo = m;
        if (typeof mo['url'] !== 'string' || mo['url'] === '')
            continue;
        if (typeof mo['addedAt'] !== 'string')
            continue;
        // 只保留合法仓库地址（拒绝 userinfo 等），损坏条目静默跳过（不拖垮整个列表）
        if (validateRepoUrl(mo['url']) !== null)
            continue;
 ...
 */
export async function writeMarketConfig(dir, cfg) {
 ...
 */
export async function addMarket(dir, url) {
    const err = validateRepoUrl(url);
    if (err !== null)
        throw new Error(err);
    const cfg = await readMarketConfig(dir);
    if (cfg.markets.some((m) => m.url === url))
 ...
 */
export async function removeMarket(dir, url) {
    const cfg = await readMarketConfig(dir);
    const next = cfg.markets.filter((m) => m.url !== url);
    if (next.length !== cfg.markets.length) {
        const updated = { schemaVersion: MARKET_CONFIG_SCHEMA_VERSION, markets: next };
