...
import { verifyChecksums, sha256Hex } from "../utils/hashing.js";
import { parseManifest, CHECKSUMS_FILE, MANIFEST_FILE } from "../schema/manifest.js";
import { validateSectionData, SECTION_JSON_PATHS, SECTION_FILE_PREFIXES, isFileSection } from "../schema/config.js";
import { parseMarketItemManifest } from "./index-parser.js";
import { MAX_MARKET_ZIP_BYTES, BANNED_MARKET_SECTIONS } from "./types.js";
 ...
 */
export function validateMarketItem(itemId, manifestRaw, zipBytes) {
    const errors = [];
    const warnings = generateSupplyChainWarnings(itemId);
    ...
    if (archive.has(CHECKSUMS_FILE)) {
        try {
            const table = JSON.parse(archive.readEntryText(CHECKSUMS_FILE));
    ...
    for (const [sectionId, enabled] of Object.entries(internalManifest.sections)) {
        if (!enabled)
            continue;
    ...
    const l3Sections = Object.entries(internalManifest.sections)
        .filter(([, on]) => on)
        .map(([id]) => id);
 ...
 */
export function generateSupplyChainWarnings(itemId) {
    return [
