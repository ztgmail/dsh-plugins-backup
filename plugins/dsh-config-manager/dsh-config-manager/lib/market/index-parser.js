...
import { SAFE_ITEM_ID_RE, MARKET_INDEX_SCHEMA_VERSION, MARKET_ITEM_SCHEMA_VERSION } from "./types.js";
...
function optString(obj, key, errors, path) {
    const v = obj[key];
    if (v === undefined)
        return undefined;
    if (typeof v !== 'string') {
...
function optStringArray(obj, key, errors, path) {
    const v = obj[key];
    if (v === undefined)
        return undefined;
 ...
 */
function parseIndexItem(raw, errors, inPath, dropped) {
 ...
 */
export function parseMarketIndex(raw) {
 ...
 */
export function parseMarketItemManifest(raw) {
    if (typeof raw !== 'string' || raw === '') {
