...
import { Phase3Recovery } from "../core/phase3-host.js";
...
function parseCliDataDir(argv) {
    const options = { command: 'recover-stale-lock', dryRun: false, profile: 'web', yes: false, list: false, wipeConfig: false };
...
export function parseCli(argv) {
    ...
    if (command === undefined)
        return { ok: false, error: '缺少子命令 / missing subcommand' };
    ...
    const options = { command, dryRun: false, profile: 'web', yes: false, list: false, wipeConfig: false };
...
export function parseSelectionInput(text) {
    ...
    for (const n of indexes)
        set.add(REINSTALL_ITEMS[n - 1].id);
    return set;
}
/** 解析 reinstall 二次确认输入：仅 "YES" 或 "Y"（忽略大小写/空白）视为确认。 */
export function parseConfirmInput(text) {
...
export function resolveDataDir(flag, env = process.env) {
...
function safeModeMarkerPath(homeDir) {
...
export function checkSafeModeBlocked(homeDir) {
    try {
...
export function pickDefaultSnapshotId(metas) {
...
export function printUsage(io = defaultIo) {
    io.log([
...
function printSnapshots(metas, io) {
    if (metas.length === 0) {
...
function printPlan(plan, io) {
    ...
    io.log(`汇总 / summary: 整文件还原 ${s.hostFileRestores} · 整文件删除 ${s.hostFileRemoves}`
...
function printReinstallPlan(plan, io) {
...
function runExternal(file, args) {
 ...
 */
function createDefaultExec(platform = process.platform) {
    return async (cmd) => {
...
async function interactiveReinstallSelect(defaults, io, ask) {
    io.log('可选清理项 / selectable items（默认勾选项带 *，[危险] 需二次确认）:');
    const lines = REINSTALL_ITEMS.map((it, idx) => {
...
async function runReinstall(options, io, env, deps) {
    const ask = deps.ask ?? defaultAsk;
    const exec = deps.exec ?? createDefaultExec(deps.platform);
    try {
        if (options.list) {
        ...
        if (options.yes) {
            ...
            const defaults = new Set(REINSTALL_ITEMS.filter((it) => it.defaultOn).map((it) => it.id));
            ...
            const hasDestructive = [...selection].some((id) => REINSTALL_ITEMS.find((it) => it.id === id)?.destructive === true);
        ...
        try {
            await runWithMutationLock(lock, { op: 'cli-reinstall', target: plan.version }, async (lockCtx) => {
                const runReinstall = async (journalCtx) => {
 ...
 */
export async function runCli(argv, io = defaultIo, env = process.env, deps = {}) {
