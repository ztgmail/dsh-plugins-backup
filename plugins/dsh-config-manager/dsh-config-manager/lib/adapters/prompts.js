...
export function extractPrompts(lines) {
    ...
    for (const line of lines) {
        for (const entry of entriesOf(line.raw)) {
            ...
            else if (sp !== null && typeof sp === 'object' && typeof sp['persona'] === 'string') {
            ...
            const pm = c['planMode'];
            if (pm !== null && typeof pm === 'object' && Array.isArray(pm['sections'])) {
                const sections = pm['sections'];
                for (const [i, s] of sections.entries()) {
                    if (s === null || typeof s !== 'object' || typeof s['text'] !== 'string' || s['text'].trim() === '')
                        continue;
                    const name = typeof s['name'] === 'string' && s['name'] !== '' ? s['name'] : `${line.lineId}:section${i}`;
                    prompts.push({ id: `prompt:${line.lineId}:${name}`, name, kind: 'planMode', text: s['text'], sourceLineId: line.lineId, sourceLineName });
                }
            }
        }
    }
...
export function mergePromptIntoLine(raw, prompt) {
    ...
    if (prompt.kind === 'systemPrompt') {
        ...
        if (typeof sp === 'string')
            config['systemPrompt'] = { persona: sp };
        else if (sp === null || typeof sp !== 'object')
            config['systemPrompt'] = { persona: prompt.text };
        else
            config['systemPrompt']['persona'] = prompt.text;
    }
    else {
        if (config['planMode'] === null || typeof config['planMode'] !== 'object')
            config['planMode'] = {};
        const pm = config['planMode'];
        if (!Array.isArray(pm['sections']))
            pm['sections'] = [];
        const sections = pm['sections'];
        const sec = sections.find((s) => s !== null && typeof s === 'object' && s['name'] === prompt.name);
        if (sec)
            sec['text'] = prompt.text;
        else
            sections.push({ name: prompt.name, text: prompt.text });
    }
    return cloned;
}
/** 由导出条目构造「重建行」raw（Create 导入用；行名取自来源行，缺失时用占位名） */
export function buildPromptLine(lineId, prompt) {
...
export class PromptsAdapter {
    ...
    async applyItem(item, ctx) {
        ...
        if (item.kind === 'Warning')
            return { ok: true, message: msg('adapter.promptWarningNoWrite') };
        const ref = item.target?.ref;
        if (!ref)
            return { ok: false, message: msg('adapter.missingTargetRef') };
        const data = ctx.sections.get('prompts');
        const prompt = data?.prompts.find((p) => p.id === item.id);
        if (!prompt)
            return { ok: false, message: msg('adapter.promptMissing', { id: item.id }) };
