...
import { isUiNamespace } from "./ui.js";
export async function resolveNamespaces(namespaces, ctx) {
    return typeof namespaces === 'function' ? namespaces(ctx) : namespaces;
}
/** 批量读取 namespace 的 redacted 记录（settings/ui 共用；单项失败跳过并告警） */
export async function collectNamespaceRecords(ctx, names, warnings) {
    const namespaces = {};
...
export async function planNamespaceItems(data, adapter, ctx) {
    const msg = ctx.msg;
    const items = [];
...
export async function applyNamespaceItem(item, sectionId, ctx) {
    const ref = item.target?.ref;
    if (!ref)
        return { ok: false, message: ctx.msg('adapter.missingTargetRef') };
    const data = ctx.sections.get(sectionId);
    const rec = data?.namespaces?.[ref];
    if (!rec)
        return { ok: false, message: ctx.msg('adapter.namespaceMissing', { ref }) };
...
export class SettingsAdapter {
    ...
    async export(ctx, _options) {
        ...
        if (nonUi.length === 0)
            warnings.push(msgOf(ctx)('adapter.noSettingsNamespaces'));
