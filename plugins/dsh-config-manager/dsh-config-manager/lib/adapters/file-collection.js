...
import { isReservedInternalRel, normalizePath } from "../utils/paths.js";
import { msgOf, zhMsg } from "../core/messages.js";
export class FileCollectionAdapter {
    ...
    async applyItem(item, ctx) {
        ...
        const data = ctx.sections.get(this.id);
        const file = data?.files.find((f) => f.relativePath === ref);
        if (!file)
            return { ok: false, message: msg('adapter.dataMissingFile', { ref }) };
