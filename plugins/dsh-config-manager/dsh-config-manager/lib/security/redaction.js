 ...
 */
import { DEFAULT_SECRET_FIELD_NAMES, isSensitiveFieldName } from "./secret-scanner.js";
...
function replaceSensitiveField(text, re, blacklist) {
    return text.replace(re, (match, field, value) => {
        if (!isSensitiveFieldName(field, blacklist))
            return match;
        if (value === '')
            return match;
 ...
 * 输出仍保留结构化形态（JSON 行合法）。
 */
export function redact(text, blacklist = []) {
    let out = text;
...
export function redactValue(value, blacklist = []) {
...
export function createRedactor(blacklist = []) {
