...
import { buildChecksums as coreBuildChecksums, verifyChecksums as coreVerifyChecksums, } from "../utils/hashing.js";
export const CHECKSUMS_ALGORITHM = 'sha256';
 ...
 */
export function parseChecksumsTable(raw, opts = {}) {
 ...
 */
export function verifyChecksums(entries, table) {
    return coreVerifyChecksums(entries, table);
}
/** 一步到位：解析 checksums.json 原文并校验（解析失败抛错；不符返回 {ok:false,...}） */
export function verifyChecksumsJson(entries, raw, opts = {}) {
...
export function describeMismatches(result) {
    const parts = [
