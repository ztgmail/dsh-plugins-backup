 ...
 */
export async function verifyRecovery(snapshot, ctx, opts) {
    const details = [];
    const manualHints = [];
    ...
    if (mismatch)
        return { verdict: 'MISMATCH', details, manualHints, at };
    if (unverifiable)
        return { verdict: 'PARTIAL_MATCH', details, manualHints, at };
    return { verdict: 'MATCH', details, manualHints, at };
}
/** 便捷：把 RecoveryVerification 映射为 journal 终态（供 verify 路由单次原子 update 使用）。 */
export function recoveryTerminalState(verdict) {
    if (verdict === 'MATCH' || verdict === 'PARTIAL_MATCH')
        return 'ROLLED_BACK';
    return 'NEEDS_ATTENTION';
}
//# sourceMappingURL=verify-recovery.js.map