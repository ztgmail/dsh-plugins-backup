...
export function resolveNpmGlobalRoot(exec) {
...
export function resolvePnpmStoreDir(env = process.env, platform = process.platform) {
    ...
    if (isWindows(platform)) {
        ...
        if (local !== undefined && local !== '')
            return path.join(local, 'pnpm', 'store');
        return null;
    }
 ...
 */
export function rmCommand(dir, platform = process.platform) {
 ...
 * 先删除目标再复制。
 */
export function copyDirCommand(src, dest, platform = process.platform) {
 ...
 */
export async function buildReinstallPlan(selection, version, exec, env = process.env, platform = process.platform) {
    const steps = [];
 ...
 */
export async function detectInstalledDshVersion(exec) {
    try {
 ...
 */
export async function writeReinstallRecoveryPoint(dataDir, point, fs = { mkdir: (p, o) => import('node:fs/promises').then((m) => m.mkdir(p, o)), writeFile: (p, c, o) => import('node:fs/promises').then((m) => m.writeFile(p, c, o)) }) {
...
export async function readReinstallRecoveryPoint(dataDir, operationId) {
    try {
