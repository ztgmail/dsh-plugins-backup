...
import { createSnapshot, FileSnapshotStore } from "./backup.js";
...
function die() { process.kill(process.pid, 'SIGKILL'); throw new Error('unreachable'); }
async function main() {
    ...
    const plan = {
        items: [{ id: 'settings:general', kind: 'Update', adapter: 'settings', description: 'u', severity: 'info', target: { adapter: 'settings', ref: 'general' } }],
        globalStrategy: 'replace', pathMappings: [], missingSecrets: [], needsRestart: false,
        estimatedActions: {},
    };
    ...
    const ctx = {
        ...
        profile: 'web',
        fs: {
            readFile: async (p) => Buffer.from('settings.yaml-content', 'utf8'),
        },
        settings: {
            describe: async (ref) => ({ value: { theme: 'dark' }, revision: 1, secrets: [] }),
        },
    ...
    try {
        await recovery.runJournaled({
            ...
            fn: async (journalCtx) => {
                if (crashPoint === 'C1')
                    die(); // after journal CREATED, before snapshot
                ...
                if (crashPoint === 'C2')
                    die(); // during/after snapshot write, before bind
                await journalCtx?.bindSnapshot(snap.id);
                if (crashPoint === 'C3')
                    die(); // after journal binding (SNAPSHOT_CREATED), before APPLYING
                await journalCtx?.markApplying();
                if (crashPoint === 'C4')
                    die(); // after APPLYING, before mutation
                // 真实 local file mutation
                await fs.mkdir(path.dirname(sideEffect), { recursive: true });
                await fs.writeFile(sideEffect, 'mutated');
                if (crashPoint === 'C5')
                    die(); // after first local file mutation
                if (crashPoint === 'C6')
                    die(); // mid-mutation
                if (crashPoint === 'C7')
                    die(); // after mutation, before COMMITTED
                // 业务成功
            },
        });
...
//# sourceMappingURL=phase4-crash-child.js.map