...
import { listSnapshots, planRestore, restore as executeRestore } from "./restore.js";
import { createEncryptionProvider } from "../security/encryption.js";
...
function assertValidSnapshotId(snapshotId) {
...
async function snapshotDirExists(deps, snapshotId) {
...
export function createModelTools(deps) {
    return {
        ...
        async backup(input) {
            ...
            return runWithMutationLock(deps.host.mutationLock, { op: 'model-backup', target: 'exports', isBlocked: () => deps.host.safeModeIsBlocked?.() ?? false }, async () => {
        ...
        async restore(input) {
            ...
            const doRestore = async () => executeRestore(restoreOpts);
            const report = await runWithMutationLock(deps.host.mutationLock, { op: 'model-restore', target: snapshotId, isBlocked: () => deps.host.safeModeIsBlocked?.() ?? false }, async (lockCtx) => {
...
export function registerModelTools(ctx, deps) {
    ...
    register(defineTool({
        name: 'config_restore',
        description: '恢复到指定快照。默认只返回动作计划（零写入）；确认要真实执行时传 confirm:true（会覆盖/删除 $DSH_HOME 文件并卸载导入期间新增插件，属破坏性操作，务必先预览）。',
        parameters: {
            snapshotId: {
                type: 'string',
                required: true,
    ...
    register(defineTool({
        ...
        description: '手动推送 DSH 配置同步到远端（Git/WebDAV），复用已持久化的通道配置。写远端属主动操作；加密/含凭据须传 password 且引擎强制 encrypt。',
        parameters: {
            channel: {
                type: 'string',
                enum: ['git', 'webdav'],
