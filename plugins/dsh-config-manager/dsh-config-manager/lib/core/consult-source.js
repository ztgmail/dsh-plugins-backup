...
import { parseZipHardened } from "../security/zip-security.js";
import { MANIFEST_FILE, CHECKSUMS_FILE, parseManifest, validateManifest } from "../schema/manifest.js";
import { verifyChecksumsJson } from "../security/integrity.js";
...
export async function readExportZipSource(ref, zipPath, opts = {}) {
    ...
    if (manifest !== null) {
        for (const [sid, enabled] of Object.entries(manifest.sections)) {
            if (!enabled)
                continue;
    ...
    for (const [sid, data] of sections) {
        ...
        for (const h of hits)
            sensitiveHits.push({ path: `${sid}:${h.path}`, field: h.field });
    }
 ...
 */
export function buildLocalSnapshotSource(ref, input) {
    const sectionFlags = {};
    for (const sid of input.sections.keys())
        sectionFlags[sid] = true;
    const manifest = {
        schemaVersion: 1,
        exporter: { name: 'DSH Config Manager', version: 'snapshot' },
        source: { dshVersion: input.sourceDsh, platform: input.sourcePlatform, arch: 'unknown' },
        exportedAt: new Date().toISOString(),
        sections: sectionFlags,
        security: { containsSecrets: false, encrypted: false, encryption: null },
    };
    const manifestIssues = input.verify.ok ? [] : [{ path: '$', message: input.verify.reason ?? '快照完整性校验失败', severity: 'error' }];
 ...
 */
export function buildProfileSource(ref, input) {
    const sectionFlags = {};
    for (const sid of input.sections.keys())
        sectionFlags[sid] = true;
    const manifest = {
...
//# sourceMappingURL=consult-source.js.map