...
import { SECTION_FILE_PREFIXES, SECTION_JSON_PATHS, isFileSection, validateSectionData, } from "../schema/config.js";
import { CHECKSUMS_FILE, MANIFEST_FILE, parseManifest } from "../schema/manifest.js";
import { canImport, describeVersion, isSupported } from "../schema/versions.js";
...
export class Analyzer {
    ...
    async loadBundle(zipPath) {
        ...
        if (archive.has(CHECKSUMS_FILE)) {
            const table = parseJsonSafe(archive.readEntryText(CHECKSUMS_FILE));
        ...
        const bundle = { archive, manifest, checksums, zipWarnings };
    ...
    async executeImportPlan(zipPath, plan, opts = {}) {
        ...
        const executed = [];
        const warnings = [...bundle.zipWarnings];
