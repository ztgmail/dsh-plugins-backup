...
export declare const MAX_RUN_LOG_LINES = 500;
/** 进行中同 kind 已有 run 时注册被拒（宿主路由 → 409）。 */
export declare class RunConflictError extends Error {
    /** 已在进行中的 run id（供客户端定位既有任务）。 */
    readonly runId: string;
    constructor(runId: string, kind: RunKind, msg?: MsgFunc);
}
/** 完成/失败后的默认保留时长（30 分钟）。 */
export declare const DEFAULT_RUN_RETENTION_MS: number;
...
export declare class RunRegistry {
    ...
    update(runId: string, patch: Partial<Pick<RunState, 'section' | 'sectionTotal' | 'item' | 'itemTotal' | 'detail'>>): RunState | undefined;
    /**
