...
import { resolveFileTarget, verifySnapshot } from "./backup.js";
import { expectedSessionRefs, sweepGhostSessions } from "./ghost-sweep.js";
import { zhMsg } from "./messages.js";
import { classifyDshPluginFailure, readInstalled, resolveProfileDir, runDshPlugin, } from "./plugin-cli.js";
...
async function loadSnapshot(snapshotDir, msg) {
...
function isWithinHome(homeDir, abs) {
    const root = path.resolve(homeDir);
    return abs === root || abs.startsWith(root + path.sep);
}
/** homeDir 内绝对路径（越界抛错） */
function homeAbs(homeDir, relPath, msg) {
    const abs = path.resolve(homeDir, relPath);
    if (!isWithinHome(homeDir, abs))
        throw new Error(msg('restore.pathEscape', { path: relPath }));
    return abs;
}
/** 快照目录内 blob 绝对路径（防 snapshot.json 里伪造 ../ 越界读） */
function blobAbs(snapshotDir, blobPath, msg) {
    const abs = path.resolve(snapshotDir, blobPath);
    if (!isWithinHome(snapshotDir, abs))
        throw new Error(msg('restore.blobPathEscape', { path: blobPath }));
    return abs;
}
/** 绝对路径 → 相对 $DSH_HOME（跨平台归一为 / 分隔；越界返回空串） */
function toRelPath(homeDir, abs) {
    if (!isWithinHome(homeDir, abs))
        return '';
    return path.relative(homeDir, abs).split(path.sep).join('/');
}
/** file 类条目的目标绝对路径（复用 backup.ts 的基准目录规则；仅读 homeDir） */
function resolveFileAbs(homeDir, adapter, ref) {
    return resolveFileTarget({ homeDir }, adapter, ref);
}
/** settings 文件 relPath：settingsPath 覆盖优先，否则探测 settings.yaml → settings.json */
async function resolveSettingsRelPath(homeDir, settingsPath) {
    ...
    for (const cand of ['settings.yaml', 'settings.json']) {
        if (await fileExists(path.join(homeDir, cand)))
            return cand;
    }
    return undefined;
}
/** 把当前文件复制到 <snapshotDir>/pre-restore/（双保险，命名含序号防碰撞） */
async function copyToPreRestore(preDir, absPath, label, seq) {
...
async function defaultPluginUninstaller(name, profileDir, profile, msg) {
...
async function assertNoSnapshotSymlink(dir, rel) {
    const p = path.join(dir, rel);
    if (!isWithinHome(dir, p))
        return `路径越界: ${rel}`;
    try {
        const st = await fs.lstat(p);
        if (st.isSymbolicLink())
            return `symlink 化关键文件: ${rel}`;
        return null;
    }
    catch {
 ...
 * 绝不把「不可证明」当成 trusted。
 */
export async function validateSnapshotForRestore(snapshotDir, snapshotsRoot, env) {
    const id = path.basename(snapshotDir);
    if (!isValidSnapshotId(id))
        return { verdict: 'INVALID', reason: 'snapshotId 非法' };
    ...
    for (const f of ['snapshot.json', 'manifest.json']) {
        ...
        if (s !== null)
            return { verdict: 'UNSAFE_PATH', reason: s };
    }
    ...
    if (selfDirGuard !== null)
        return { verdict: 'UNSAFE_PATH', reason: selfDirGuard };
    const blobsDirGuard = await assertNoSnapshotSymlink(snapshotDir, 'blobs');
    if (blobsDirGuard !== null)
        return { verdict: 'UNSAFE_PATH', reason: blobsDirGuard };
    ...
    const blobEntries = await fs.readdir(blobsDir, { withFileTypes: true }).catch(() => []);
    ...
    let snapshot = null;
    try {
        snapshot = parseJsonSafe(await fs.readFile(path.join(snapshotDir, 'snapshot.json'), 'utf8'));
    }
    catch {
        /* verifySnapshot 已报缺失 */
    }
    if (snapshot === null || typeof snapshot !== 'object')
        return { verdict: 'CORRUPT', reason: 'snapshot.json 无法解析' };
    ...
    if (!v.ok)
        return { verdict: 'CORRUPT', reason: v.reason };
 ...
 */
export async function planRestore(opts) {
    ...
    for (const a of actions) {
        switch (a.kind) {
            case 'hostFileRestore':
                ...
                break;
            case 'hostFileRemove':
                summary.hostFileRemoves += 1;
                break;
            case 'pluginRemove':
                summary.pluginRemoves += 1;
                break;
            case 'fileRestore':
                summary.fileRestores += 1;
                break;
            case 'fileRemove':
                summary.fileRemoves += 1;
                break;
            case 'credentialHint':
                summary.credentialHints += 1;
                break;
            case 'skip':
                summary.skips += 1;
                break;
...
export async function restore(opts) {
    ...
    const uninstaller = opts.pluginUninstaller
        ?? ((name, dir, prof) => defaultPluginUninstaller(name, dir, prof, msg));
    const report = {
    ...
    try {
        ...
        if (ghostSessions.length > 0) {
            report.skipped.push(msg('restore.ghostSweepSummary', { count: String(ghostSessions.length), keys: ghostSessions.join(', ') }));
        }
    }
    catch (err) {
 ...
 */
async function listSessionEntries(homeDir) {
 ...
 */
async function sweepRestoredSessions(snapshotDir, homeDir, msg) {
    const snapshot = await loadSnapshot(snapshotDir, msg);
 ...
 *  不存在视为成功（幂等）。返回是否实际删除。 */
export async function deleteSnapshot(snapshotsDir, id) {
    if (!isValidSnapshotId(id)) {
 ...
 *  只接受合法 id；快照不存在抛错。 */
export async function setSnapshotPinned(snapshotsDir, id, pinned) {
    if (!isValidSnapshotId(id)) {
        throw new Error(`非法快照 id: ${JSON.stringify(id)}`);
    }
    ...
    const snapshot = parseJsonSafe(await fs.readFile(file, 'utf8'));
