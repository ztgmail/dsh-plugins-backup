...
async function isExpired(target, retentionMs, nowMs) {
    try {
...
async function removeEntry(target, label, result) {
    try {
        let size = 0;
        try {
            const st = await fs.stat(target);
 ...
 */
export async function cleanupCaches(opts) {
    ...
    const result = { removed: 0, freedBytes: 0, errors: 0, detail: [] };
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            const isTmpish = (entry.isFile() && entry.name.endsWith('.zip')) ||
    ...
    try {
        ...
        for (const entry of entries) {
            if (!entry.isFile() || !entry.name.endsWith('.zip'))
                continue;
            if (exemptPrefix !== undefined && entry.name.startsWith(exemptPrefix))
                continue;
    ...
    try {
        ...
        for (const hash of hashes) {
            ...
            const itemDirs = await fs.readdir(itemsDir).catch(() => []);
            ...
            const itemsLeft = await fs.readdir(itemsDir).catch(() => []);
            if (itemsLeft.length === 0) {
                await removeEntry(itemsDir, `market/cache/${hash}/items`, result);
            }
            const remaining = await fs.readdir(hashDir).catch(() => []);
...
//# sourceMappingURL=cache-cleaner.js.map