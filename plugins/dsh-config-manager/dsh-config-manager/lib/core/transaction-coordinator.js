 ...
 */
import { JournalStore, createJournalEntry, transitionJournalState, isTerminalState, generateOperationId, redactJournalText, } from "./journal.js";
// ---------- Coordinator ----------
export class MutationTransactionCoordinator {
    ...
    async run(op) {
        ...
        if (!activeCheck.clear) {
            ...
            const recovered = this.deps.reconcileResidue
                ? await this.deps.reconcileResidue(activeCheck.residue, context).catch(() => 'needs-attention')
                : 'needs-attention';
        ...
        let journal = await this.deps.store.create(entry);
        let snapshotId = null;
        try {
            // 4. snapshot
            ...
            for (let i = 0; i < plannedSteps.length; i++) {
                ...
                try {
                    ...
                    if (result.status === 'attention') {
                        needAttention = true;
                        if (result.warning)
                            attentionReasons.push(result.warning);
                    }
                }
                catch (err) {
     ...
     */
    async rollbackForRecovery(op, context, operationId, snapshotId, cause) {
    ...
    async doRollback(op, context, operationId, snapshotId, cause) {
        try {
            await this.deps.store.update(operationId, (j) => {
            ...
            const report = await op.rollback(snapshotId, context, async (entryIndex) => {
                await this.deps.store.update(operationId, (j) => {
                    if (!(j.state === 'ROLLING_BACK' || j.state === 'NEEDS_ATTENTION'))
                        return j;
    ...
    async finalizeAttention(operationId, reason, snapshotId, context) {
