 ...
 */
async function backupHostFiles(ctx, blobs) {
...
export function resolveFileTarget(ctx, adapter, ref) {
 ...
 */
export function resolveFileTargetRel(adapter, ref) {
    const base = FILE_BASES[adapter] ?? '';
...
function collectTargets(plan) {
    ...
    for (const item of plan.items) {
        if (!EXECUTABLE_KINDS.has(item.kind) || item.target === undefined)
            continue;
...
async function engineSnapshotEntry(ctx, target) {
...
export async function createSnapshot(opts) {
    ...
    for (const adapter of adapters) {
        ...
        if (!adapterTargets || adapterTargets.length === 0)
            continue;
        if (adapter.snapshot) {
...
export function selectPruneCandidates(metas, limit = SNAPSHOT_RETENTION_LIMIT) {
...
export class FileSnapshotStore {
    ...
    async save(snapshot, blobs = new Map()) {
        ...
        for (const [blobPath, data] of blobs) {
            ...
            if (!target.startsWith(dir))
                throw new Error(`快照 blob 路径越界: ${blobPath}`);
            await fs.mkdir(path.dirname(target), { recursive: true });
            await atomicWriteFile(target, data);
        }
        // 2) 写 snapshot.json（readiness='CREATING'，未完成不可用）
        const creating = { ...snapshot, readiness: 'CREATING' };
   ...
   *  必须豁免——引用提供者（referencedSnapshotIds）返回的 id 绝不自动清理。 */
    async prune() {
        ...
        const referenced = await this.options.referencedSnapshotIds?.().catch(() => new Set()) ?? new Set();
        const metas = [];
        for (const entry of entries) {
            ...
            try {
                ...
                if (typeof parsed.id !== 'string' || parsed.id === '' || typeof parsed.createdAt !== 'string')
                    continue;
                if (parsed.pinned === true)
                    continue; // 置顶快照豁免自动清理
                if (referenced.has(parsed.id))
                    continue; // Phase 4 F3：recovery 引用豁免
        ...
        for (const id of selectPruneCandidates(metas)) {
            ...
            if (!target.startsWith(dir))
                continue; // 越界 id 跳过（不删、不抛，同 save/readBlob 包含性约定）
        ...
        if (removedIds.length > 0) {
            try {
                this.options.onPrune?.(removedIds);
            }
            catch { /* 历史回调失败不阻断 */ }
        }
    }
 ...
 */
export function computeMetadataHash(snapshot) {
    const destructive = {
 ...
 */
export async function verifySnapshot(snapshotsDir, id) {
    ...
    const manifest = parseJsonSafe(manifestRaw);
    ...
    for (const [blobPath, expectedHash] of Object.entries(manifest.blobHashes)) {
        const target = path.join(dir, blobPath);
        if (!target.startsWith(dir))
            return { ok: false, reason: `blob 路径越界: ${blobPath}` };
        let data;
        try {
    ...
    for (const e of snapshot.entries) {
        if (e.kind === 'file' && e.copiedTo && e.copiedTo !== '')
            referencedBlobs.push(e.copiedTo);
    }
    for (const hb of snapshot.hostFileBackups ?? []) {
        if (hb.blobPath && hb.blobPath !== '' && hb.existed)
            referencedBlobs.push(hb.blobPath);
    }
