...
import { ProfileManager, isValidProfileName } from "./profiles/index.js";
...
import { BackupScheduler } from "./sync/backup-scheduler.js";
import { readBackupSchedule, writeBackupSchedule } from "./sync/backup-schedule-config.js";
import { AUTO_BACKUP_PREFIX, deleteBackupFile, isValidBackupFileName, isValidExportFileName, listBackupFiles, resolveNonCollidingExportName, writeBackupNote } from "./sync/backup-files.js";
import { validateBackupScheduleDraft } from "./ui/backup-schedule.js";
...
import { GitMarketReader } from "./market/reader.js";
import { parseMarketIndex, parseMarketItemManifest } from "./market/index-parser.js";
import { BUILTIN_MARKET_URL, isOfficialMarket } from "./market/builtin.js";
import { validateMarketItem } from "./market/security.js";
import { prepareMarketItem } from "./market/prepare.js";
import { validateMarketRepoUrl } from "./market/url.js";
import { marketItemWarnings, toMarketListItem } from "./market/view.js";
import { GitHubAuthRest, GitHubApiError } from "./market/github-repos.js";
import { MyRepoError, MyRepoService, USER_CONFIGS_REPO, userConfigsRepoUrl } from "./market/my-repo.js";
import { createGitFileWriter } from "./market/git-file-writer.js";
import { parseGitHubRepoUrl } from "./market/repo-url.js";
import { StarCache } from "./market/star-cache.js";
...
class DshFileSystemFacade {
    ...
    async listRecursive(dir) {
        ...
        const walk = async (current) => {
            if (!isSameOrChild(current, this.homeDir))
                return;
            let entries;
            try {
...
function makeRoutes(deps) {
    ...
    const selectionViewByChannel = async () => {
        ...
        selectionCache.git = all.git;
        selectionCache.webdav = all.webdav;
        const view = (sel) => ({ mode: sel.mode, sections: sel.sections, encrypt: sel.encrypt, includeSecrets: sel.includeSecrets });
    ...
    const urlHash = (url) => sha256Hex(url).slice(0, 32);
    const marketCacheIndex = (url) => join(marketCacheRoot, urlHash(url), 'index.json');
    const marketCacheItemDir = (url) => join(marketCacheRoot, urlHash(url), 'items');
    const marketWorkDir = (url) => join(marketWorkRoot, urlHash(url));
    /**
    ...
    const makeMarketReader = () => new GitMarketReader({ msg, timeoutMs: 60_000 });
    /**
    ...
    async function buildMarketSummary(e) {
        const s = { url: e.url, addedAt: e.addedAt };
    ...
    async function itemCached(url, itemId) {
        try {
    ...
    async function writeItemCache(url, itemId, manifestRaw, zipBytes) {
    ...
    async function pruneStagedMarketZips() {
        try {
            const names = await fs.readdir(tmpDir);
            const now = Date.now();
            for (const name of names) {
                // market-*.zip（未确认导入的暂存）与 publish-*.zip（发布向导产物）都纳入保留策略
                if ((!name.startsWith('market-') && !name.startsWith('publish-')) || !name.endsWith('.zip'))
                    continue;
                const p = join(tmpDir, name);
                try {
                    const st = await fs.stat(p);
                    if (now - st.mtimeMs > MARKET_TMP_RETENTION_MS)
    ...
    const routesList = [
        ...
        // ------------------------------------------------------------- export
        {
            ...
            handler: async (req, res) => {
                ...
                const only = Array.isArray(body['only'])
                    ? body['only'].filter((x) => typeof x === 'string' && SECTION_IDS.includes(x))
                    : undefined;
                ...
                try {
                    ...
                    const result = await withTimeout(exporter.export({ includeSecrets, only, outPath: plainZipPath }), ROUTE_TIMEOUT_MS, msg('host.exportTimeout'));
        ...
        // ------------------------------------------------------------- execute
        {
            ...
            handler: withMutationGate('import-apply', async (req, res, lockCtx, journalCtx) => {
                ...
                const decryptPassword = typeof opts['decryptPassword'] === 'string' && opts['decryptPassword'] !== ''
                ...
                try {
                    ...
                    const result = await makeImporter().executeImportPlan(zipPath, plan, {
                        ...
                        secretInputs: opts['secretInputs'] !== null && typeof opts['secretInputs'] === 'object'
                            ? opts['secretInputs']
                            : {},
                    ...
                    const importSections = Array.from(new Set(plan.items.map((i) => i.adapter).filter((s) => typeof s === 'string' && s !== '')));
        ...
        },
        {
            ...
            handler: withMutationGate('profile-save', async (req, res) => {
                ...
                const sections = Array.isArray(body['sections'])
                    ? body['sections'].filter((x) => typeof x === 'string' && SECTION_IDS.includes(x))
                    : undefined;
                try {
                    ...
                    const historyError = await tryAppendHistory({
                        kind: 'profile-save',
                        result: 'success',
                        sections: Array.isArray(meta?.sections) ? meta.sections.filter((s) => typeof s === 'string') : [],
                        source: 'api',
                        summary: `保存配置档案 ${name}`,
                    });
        ...
        },
        {
            ...
            handler: withMutationGate('profile-delete', async (req, res) => {
                if (!guard(req, res, 'POST'))
                    return;
                const body = await readJsonBody(req);
                const name = typeof body === 'object' && body !== null ? body['name'] : undefined;
        ...
        },
        {
            ...
            handler: withMutationGate('profile-switch', async (req, res, lockCtx, journalCtx) => {
                ...
                const secretInputs = body['secretInputs'] !== null && typeof body['secretInputs'] === 'object'
                    ? body['secretInputs']
                    : {};
                const resolutions = body['resolutions'] !== null && typeof body['resolutions'] === 'object'
                    ? body['resolutions']
                    : {};
                const strategy = body['strategy'] === 'replace' || body['strategy'] === 'skipExisting' ? body['strategy'] : 'merge';
                ...
                try {
                    ...
                    const switchSections = result.sections ?? [];
                    const historyError = await tryAppendHistory({
                        kind: 'profile-switch',
                        result: 'success',
        ...
        // 统一咨询报告。**只读**：不写配置/快照/journal；临时 ZIP 用 try/finally 立即清理。
        {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    const computeMigratability = async (zipPath) => {
                        try {
                    ...
                    if (type === 'export-zip') {
                        ...
                        for (const e of snapshot?.entries ?? []) {
                            if (e.adapter !== undefined)
                                snapshotSections.set(e.adapter, {});
                        }
                        let restorePlan = { itemCount: 0, conflicts: 0, warnings: 0, sections: [], errors: [] };
                        try {
                        ...
                        let switchPreview = { itemCount: 0, conflicts: 0, warnings: 0, sections: [], errors: [] };
                        try {
        ...
        // sections 可选（高级/自定义导出）：只推送勾选的分区；缺省 = 默认模式全部推荐分区。
        {
            ...
            handler: withMutationGate('sync-push', async (req, res) => {
                ...
                try {
                    ...
                    const includeSecrets = body['includeSecrets'] === true;
                    const encryptPassword = typeof body['encryptPassword'] === 'string' && body['encryptPassword'] !== ''
                    ...
                    const report = preview
                        ? await withTimeout(engine.previewPush({
                            ...(sections === undefined ? {} : { sections }),
        ...
        // 绝不直接写配置、绝不执行导入（executeImportPlan 由上层按用户确认驱动）。
        {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    const decryptPassword = typeof body['decryptPassword'] === 'string' && body['decryptPassword'] !== ''
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    const decryptPassword = typeof body['decryptPassword'] === 'string' && body['decryptPassword'] !== ''
                        ? body['decryptPassword']
                        : undefined;
                    const preview = await withTimeout(engine.preview({
                        ...(snapshotId === undefined ? {} : { snapshotId }),
                        ...(decryptPassword === undefined ? {} : { password: decryptPassword }),
                    }), ROUTE_TIMEOUT_MS, msg('host.syncPullTimeout'));
        ...
        // ---------------------------------------------------- market/browse
        {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    let index = await readCachedIndexObj(url);
                    if (index === null) {
                        const reader = makeMarketReader();
                        ...
                        if (cacheState === 'cached') {
                            try {
                                ...
                                if (parsed.ok && parsed.manifest !== null)
                                    sections = parsed.manifest.sections;
                            }
                            catch {
        ...
        // 真正落盘由用户确认后走既有 POST /execute（zipPath + plan）。零写入到确认。
        {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    const analysis = await importer.analyzeImport(zipPath);
                    const plan = await importer.createImportPlan(zipPath, { strategy: 'merge', resolutions: {}, pathMappings: [] });
                    const download = { ...base, zipPath, analysis, plan };
        ...
        // 零写入配置：只在受控临时区生成发布目录；插件不做任何 git 写操作、不持有凭据。
        {
            ...
            handler: async (req, res) => {
                ...
                const categories = Array.isArray(categoriesRaw)
                    ? categoriesRaw.filter((c) => typeof c === 'string')
                    : undefined;
        ...
        // 禁用 withMutationGate（避免 double-journal）；mutation 路由经 withMutationLock + loopback fence。
        {
            ...
            handler: async (req, res) => {
                ...
                if (!isValidOperationId(operationId)) {
                    writeJson(res, 400, { error: 'invalid operationId' });
                    return;
                }
                const methodFor = { preview: 'GET', confirm: 'POST', execute: 'POST', verify: 'POST', retry: 'POST', dismiss: 'POST' };
                ...
                try {
                    ...
                    await runWithMutationLock(host.mutationLock, { op: `recovery-${action}`, target: operationId }, async () => {
                        ...
                        let r;
                        if (action === 'confirm')
                            r = await recoveryOrchestrator.confirm(operationId, userConfirmed);
                        else if (action === 'execute')
                            r = await recoveryOrchestrator.execute(operationId, userConfirmed, makeRecoveryExecutors);
                        else if (action === 'verify')
                            r = await recoveryOrchestrator.verify(operationId);
                        else if (action === 'retry')
                            r = await recoveryOrchestrator.retry(operationId, userConfirmed, makeRecoveryExecutors);
                        else if (action === 'dismiss')
                            r = await recoveryOrchestrator.dismiss(operationId, userConfirmed);
                        else
                            r = { status: 404, body: { error: 'not found' } };
