 ...
 */
import { CURRENT_SCHEMA_VERSION, MIN_SUPPORTED_SCHEMA_VERSION, UnsupportedSchemaError } from "../schema/versions.js";
import { V1_TO_V2 } from "./v1-to-v2.js";
/**
 * 迁移注册表：按 from 升序的链式步骤。
 * 当前只有 v1→v2 占位（CURRENT=1 时不会触发，schema v2 发布后生效）。
 */
export const MIGRATIONS = [V1_TO_V2];
...
export function registerMigration(step) {
    ...
    if (dup)
        throw new Error(`迁移步骤冲突: ${step.from}→${step.to} 与 ${dup.from}→${dup.to} 重叠`);
 ...
 * - 无可用迁移路径 → 抛错。
 */
export function migrateToCurrent(doc, fromVersion, targetVersion = CURRENT_SCHEMA_VERSION) {
