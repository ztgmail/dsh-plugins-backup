 ...
 */
function validateSnapshotDocument(value) {
    ...
    for (const file of snap.files) {
        ...
        if (typeof entry.path !== 'string'
            || !SNAPSHOT_FILES.includes(entry.path)
            || entry.path.includes('..')) {
 ...
 */
function isTrulyAbsent(path, readError) {
...
function captureError(path, reason, reasonZh) {
...
function captureReadError(path, error) {
...
function ensureSafeRestoreParent(profileDir, parent, snapshotPath) {
...
function snapshotIdOrderKey(id) {
...
export function compareSnapshotIdsNewest(a, b) {
    const aKey = snapshotIdOrderKey(a);
    const bKey = snapshotIdOrderKey(b);
    if (aKey.category !== bKey.category)
        return aKey.category - bKey.category;
    if (aKey.primary !== bKey.primary)
        return aKey.primary < bKey.primary ? 1 : -1;
    if (aKey.sequence !== bKey.sequence)
        return aKey.sequence < bKey.sequence ? 1 : -1;
    return aKey.fallback === bKey.fallback ? 0 : aKey.fallback < bKey.fallback ? 1 : -1;
}
function pruneSnapshotsKeeping(profileDir, max, keepId) {
...
export function createProfileSnapshot(profileDir, maxSnapshots = DEFAULT_MAX_SNAPSHOTS) {
    ...
    for (const path of SNAPSHOT_FILES.slice(1)) {
        ...
        try {
            ...
            if (isTrulyAbsent(absolutePath, error))
...
export function listSnapshots(profileDir) {
    ...
    for (const name of names) {
        try {
            ...
            if (validated.ok && validated.value.snapshot.id === id)
                snapshots.push(validated.value.snapshot);
        }
...
export function restoreSnapshot(profileDir, id) {
    ...
    if (!validated.ok)
        return { ok: false, restored: [], error: validated.error };
